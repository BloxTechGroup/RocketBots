...
class RocketBot {
  ...
  async info() {
    ...
    return response.success
      ? response.response
      : { success: false, reason: response.reason };
  }

  async getJoinRequests() {
    const response = await request(
      `${api}/${this.teamId}/getjoinrequests`,
      "GET",
      {},
      {}
    );
  ...
  async approveJoinRequest(userId) {
    const response = await request(
      `${api}/${this.teamId}/approvejoinrequest`,
      "POST",
      { "Content-Type": "application/json" },
      JSON.stringify({
        userId: userId,
  ...
  async declineJoinRequest(userId) {
    const response = await request(
      `${api}/${this.teamId}/declinejoinrequest`,
      "POST",
      { "Content-Type": "application/json" },
      JSON.stringify({
        userId: userId,
  ...
  async getIdFromUsername(username) {
    const response = await request(
      `${api}/${this.teamId}/getidfromusername/${username}`,
      "GET",
      {},
      {}
    );
    return response.success
      ? { success: true, response: response.response }
      : { success: false, reason: response.reason };
  }
}
